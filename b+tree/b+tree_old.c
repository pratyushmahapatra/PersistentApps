...
void transferLeftNode(long *path, int position) {
    ...
    struct Node* leftNode = parent->ptr[indexKey - 1];
    leftPath[leftPathPos] = parent;
	leftPathPos++;
	leftPath[leftPathPos] = leftNode;
	leftPathPos++;
    ...
    do { 
    	...
    	if (i > leftPathLen) 
    		break;
    	leftNodeParent = (struct Node*)leftPath[leftPathLen - i];
    	indexKey = 0;
		flag = 0;
	...
	} while(indexKey == (leftNodeParent->n - 1));
}
...
void mergeLeftNode(long *path, int position) {
	...
	if (nthPtr) {
    	...
    	for (int i = indexKey - 1; i < parent->n; i++) { //FIXME
	...
	if ( parent->n < (THRESHOLD + 1)/2) {
	    if (leftNodeExists(path, position - 1) > (THRESHOLD + 1)/2) {
	        //transfer element from left node
	        transferLeftNode(path, position - 1);
	    }
	    else if (rightNodeExists(path, position - 1) > (THRESHOLD + 1)/2) {
	        //transfer element from right node
	        transferRightNode(path, position - 1);
	    }
	    else if ((leftNodeExists(path, position - 1) <= (THRESHOLD + 1)/2) && (leftNodeExists(path, position - 1) != 0)) {
	        //merge with left node
	        mergeLeftNode(path, position - 1);
	    }
	    else if (rightNodeExists(path, position - 1) <= (THRESHOLD + 1)/2 && (rightNodeExists(path, position - 1) != 0)) {
	        //merge with right node
	        mergeRightNode(path, position - 1);
	    }
    }
    int KeyIndex;
    if (leftNode->n == THRESHOLD + 1)
    	KeyIndex = leftNode->n - 2;
    else
    	KeyIndex = leftNode->n - 1;

    long key = leftNode->key[KeyIndex];
	i  = 0;
    do { 
    	i++;
    	if (i > leftPathLen) 
...
void transferRightNode(long *path, int position) {
	...
	long rightPath[1024];
	int rightPathPos = 0;
    struct Node* rightNodeParent = NULL;
    struct Node* rightNode = parent->ptr[indexKey + 1];
    rightPath[rightPathPos] = parent;
	rightPathPos++;
	rightPath[rightPathPos] = rightNode;
	rightPathPos++;
    
    while ( rightNode->leaf != true) {
    	rightNode = rightNode->ptr[rightNode->n - 1];
    	rightPath[rightPathPos] = rightNode;
		rightPathPos++;
    }
    int rightPathLen = rightPathPos;
    ...
    do { 
    	...
    	if (i > position) 
    		break;
    	nodeParent = (struct Node*)path[position - i];
    	indexKey = 0;
		flag = 0;
		for (int j = 0 ; j < nodeParent->n; j++) {
		    if ( key <= nodeParent->key[j]) {
	    		indexKey = j;
	      		flag = 1;
	        	break;
	    	}
		...
		if (flag == 0)
	    	indexKey = nodeParent->n;
    	nodeParent->key[indexKey] = node->key[node->n-1];
	} while(indexKey == (nodeParent->n - 1));
}
...
void insert(long key, char* data){
    ...
    for (int i = 0; i < node->n; i++){
        if (node->key[i] == key){
        	struct Data *new_data = (struct Data *) (node->ptr[i]);
        	memcpy(new_data->val, data, DATASIZE);
            return;
        }
    }
...
void reconstruct_list(){
	...
	while(node != NULL) {
		...
		leafNodeCnt++;
		if (largestNodebyAddr < node)
			largestNodebyAddr = node;
		if (node->n == 0)
			break;
		node = node->ptr[node->n];
	}
	...
	do
	{
		...
		for (int i = 0; i < imdNodeCnt; i++){
			struct Node* imdNode = (struct Node*)(largestNodebyAddr) + num_nodes;
			imdNode->n = 0;
			num_nodes++;
