...
void flush(long data, long addr) {
    ...
    *addr_new = data;
    ...
    msync(addr_new, sizeof(data), MS_SYNC);
    flush_count++;
...
void create(int data){
    ...
    INIT->data = 1;
    flush(INIT->data, &INIT->data);
    INIT->prev = NULL;
    flush(INIT->prev, &INIT->prev);
    struct Node* new_node = (struct Node*)(segmentp) + 1;
    new_node->data = data;
    flush(data, &new_node->data);
    new_node->prev = NULL;
    flush(new_node->prev, &new_node->prev);
    new_node->next = NULL;
    flush(new_node->next, &new_node->next);
    HEAD = new_node;
    INIT->next = HEAD;
    ...
    while (node->next != NULL){
        if (node->data == data) {
            if (node == HEAD) {
                ...
                if (newHEAD != NULL) {
                    newHEAD->prev = NULL;
                    flush(newHEAD->prev, &newHEAD->prev);
                    HEAD = newHEAD;
                    INIT->next = HEAD;
                    flush(INIT->next, &INIT->next);
                    fence();
                    break;
                }
                else {
                ...
                flush(next_node->prev, &next_node->prev);
                fence();
                break;
            }
        }
